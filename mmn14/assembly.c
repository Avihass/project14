    ...
    if (!haveError) {
        ...
        while (argIndx < argc) {
                ...
                if (firstWordType != entry_line && firstWordType != instruction_line &&
                            ...
                            if (foundedSign) {
                                ...
                                if (foundedSign->dataType != macro_sign) {
                                    ...
                                    if (actualInstruct.srcOp.type == index_met) {
                                        ...
                                        /* if true the operand is a macro */
                                        if (strcmp(actualInstruct.srcOp.indexName, "") != 0)
                                            findSign(signTabHead, actualInstruct.srcOp.indexName, macro_sign, &(actualInstruct.srcOp.val));
                    ...
                    if (actualInstruct.destOp.type != -1) {
                        ...
                        if (actualInstruct.destOp.type == imediate_met) {
                            ...
                            /* if true the operand is a macro */
                            if (strcmp(actualInstruct.destOp.macroName, "") != 0)
                                findSign(signTabHead, actualInstruct.destOp.macroName, macro_sign, &(actualInstruct.destOp.val));
                            ...
                            if (foundedSign) {
                                ...
                                if (foundedSign->dataType != macro_sign) {
                                    ...
                                    if (actualInstruct.destOp.type == index_met) {
                                        ...
                                        /* if true the index is a macro */
                                        if (strcmp(actualInstruct.destOp.indexName, "") != 0)
                                            findSign(signTabHead, actualInstruct.destOp.indexName, macro_sign, &(actualInstruct.destOp.val));
            ...
            if (!haveError) {
                ...
                signTabPtr signIndx;
                dataTabPtr codeIndex;
                specAdressWord actualSpecWord;
                
                /* remove ".as" */
                strcpy(srcFileName, argv[argIndx]);
                
                /* create .ent file (only if there is entry to write) */
                ...
                /* create .ext file (only if there is external to write) */
                signIndx = signTabHead;
                ...
                if (DC > 0) {
                    ...
                    while (codeIndex != NULL) {
                        ...
                        binToSpec(codeIndex->binWord, actualSpecWord);
                        writeToObjectFile(srcFileName, codeIndex->adress + IC, actualSpecWord);
                        codeIndex = codeIndex->next;
                    }
                }
